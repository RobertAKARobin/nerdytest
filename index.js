...
Suite.prototype = {
	...
	test: function test(/*instance.inputs.., instance.comparision*/){
		...
		test.comparators = Array.from(arguments)
		...
		test.comparators.forEach((comparator, index)=>{
			const comparatorValue = test.comparatorValues[index]
			messageParts.push(`\t${comparator.toString()}`)
			if(test.status !== 'PASS'){
				messageParts.push(`\t\t= ${comparatorValue instanceof Error ? comparatorValue.name : comparatorValue}`)
			}
		})
